...
The **Architect** owned the orchestrator and the DAG engine. **SubProc** owned the nine sub-processors. **TestEngineer** owned fixtures, the test driver, and the end-to-end suite. **Infra** owned Kubernetes manifests, CI, and the deployment story. **Dashboard** owned the Vue 3 SPA when its phase came up. **Skeptic** was the gate keeper, with a different agent type from everyone else, whose only job was to read the code and disagree with it.
